...
func DescribeBlueprint(c *cli.Context) {
	...
	bp := fetchBlueprint(c.Int64(FlOrganization.Name), c.String(FlName.Name), cbClient.Cloudbreak.V3OrganizationIDBlueprints)
...
func DeleteBlueprint(c *cli.Context) {
	...
	cbClient := NewCloudbreakHTTPClientFromContext(c)
	deleteBlueprintsImpl(cbClient.Cloudbreak.V3OrganizationIDBlueprints, c.Int64(FlOrganization.Name), c.String(FlName.Name))
}
...
func ListBlueprints(c *cli.Context) {
	...
	cbClient := NewCloudbreakHTTPClientFromContext(c)
	output := utils.Output{Format: c.String(FlOutputOptional.Name)}
	organization := FlOrganization.Name
